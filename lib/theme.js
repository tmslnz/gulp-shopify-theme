...
const LOG_COLOR = gutil.colors.inverse(' ', PLUGIN_NAME, ' ');
const ERROR_COLOR = gutil.colors.white.bgRed(' ', PLUGIN_NAME, ' ');
...
module.exports = class ShopifyTheme extends EventEmitter {
    constructor(options) {
        super();
    ...
    _handleTaskError (error, file) {
        ...
        case 'ETIMEDOUT':
        case 'ECONNRESET':
            gutil.log(LOG_COLOR, 'retry: ' + gutil.colors.yellow(this._makeAssetKey(file)));
            this._addTask(file);
            break;
        // Unprocessable entity
        case 422 :
            gutil.log(ERROR_COLOR, 'syntax error or wrong path at ' + gutil.colors.red(this._makeAssetKey(file)));
            break;
        // Invalid request
        case 406 :
        case 403 :
            gutil.log(ERROR_COLOR, 'invalid request at ' + gutil.colors.red(this._makeAssetKey(file)))
            break;
        // API Request is not valid for this shop
        case 401 :
            gutil.log(ERROR_COLOR, 'invalid shop. Check your config.')
            break;
        default:
        }
    }
    ...
    _makeConsumer (file) {
        ...
        return function (next) {
            return _this._runAssetTask(file)
                .then(function (res) {
                    gutil.log(LOG_COLOR, file.action + ':', gutil.colors.green(_this._makeAssetKey(file)));
    ...
    */
            ...
            function end (error) {
                ...
                Promise.all(taskPromisers)
                    .then((responses)=>{
                        ...
                        gutil.log(LOG_COLOR, gutil.colors.bold('Done'));
                    })
                    .catch((error)=>{
    ...
    _addTask (file) {
        ...
        gutil.log(LOG_COLOR, 'queued: ' + gutil.colors.yellow(key));

        for (var index = 0; index < this._taskQueue.length; index++) {
            if (this._taskQueue[index].key === key) {
                gutil.log(LOG_COLOR, 'requeued: ' + gutil.colors.yellow(key));
